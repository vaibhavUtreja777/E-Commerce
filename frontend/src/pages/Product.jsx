...
const Product = () => {
  ...
  useEffect(() => {
    const getProduct = async () => {
      setLoading(true);
      setLoading2(true);
      const res1 = await Axios.get('http://127.0.0.1:5000/product')
      const res = res1.data
      for(let cat of Object.keys(res)){
        for(let product of res[cat]){
          if(product.Name === id ){
            setProduct(product)
            setSimilarProducts(res[cat])
            setCategory(cat)
          }
        }
      }
        ...
        <div className="container my-5 py-2">
          <div className="row">
            ...
            <div className="col-md-6 col-md-6 py-5">
              ...
              <p className="lead">{product.Fabric} {product.Color}</p>
              <button
        ...
        <div className="py-4 my-4">
          <div className="d-flex">
            {similarProducts.map((item) => {
              return (
                <div key={item.Link} className="card mx-4 text-center">
                  <img
